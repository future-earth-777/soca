...
module soca_diffop_mod
  ...
  type, public :: soca_diffop_type
   ...
   contains
     procedure :: setup => soca_diffop_setup
     procedure :: delete => soca_diffop_delete
     procedure :: grad => soca_grad
     procedure :: sfc_geo => soca_barotropic_geostrophy
     procedure :: geostrophy => soca_geostrophy
  end type soca_diffop_type
...
contains
  ...
  subroutine soca_diffop_setup(self, geom, h)
    class(soca_diffop_type), intent(inout) :: self
  ...
  end subroutine soca_diffop_setup

  ! ------------------------------------------------------------------------------
  !> Cleanup
  subroutine soca_diffop_delete(self)
    class(soca_diffop_type), intent(inout) :: self
  ...
  end subroutine soca_diffop_delete

  ! ------------------------------------------------------------------------------
  !> Barotropic currents
  subroutine soca_barotropic_geostrophy(self, ssh, us_geo, vs_geo, geom)
    class(soca_diffop_type),              intent(in) :: self
  ...
  subroutine soca_geostrophy(self, h, t, s, dt, ds, dug, dvg, geom)
    class(soca_diffop_type),              intent(in) :: self
  ...
  end subroutine soca_geostrophy
